...
function getMediaSrc(elem){
    ...
    if(rs.length > 0){
        var _media = rs[0],
            _mediaSrc = $(_media).attr('src') || null,
...
function showMenuContext(elem){
    ...
    chrome.runtime.sendMessage({action:'show-contextMenuInstagram',
        data : {
            src : _mediaSrc,
            type : _mediaType
        }});
...
chrome.runtime.onMessage.addListener(function(msg, sender, sendResponse){
    if(msg.action === 'tabUpdated'){
        if(window.location.href === msg.data.pageUrl && msg.data.requestMethod === 'GET'){
